    ...
    def build_model(cls, args, task):
        ...
        if args.share_all_embeddings:
            ...
            # if args.encoder_embed_dim != args.decoder_embed_dim:
                ...
                # )
            if args.encoder_embed_dim != args.latent_dim: # 感觉这个才是对的，上面注释是原版条件
                raise ValueError(
                    "--share-all-embeddings requires --encoder-embed-dim to match --latent-dim"
                )
            ...
            encoder_embed_tokens = cls.build_embedding(
                args, src_dict, encoder_embed_dim, args.encoder_embed_path
            )
        ...
        if latent_dim != model_dim:
            encoder_project_in_dim = nn.Linear(encoder_embed_dim, encoder_embed_dim, bias=False)
            # decoder_project_in_dim = (
            #     encoder_project_in_dim if args.share_project_in_dim
            #     else nn.Linear(latent_dim, model_dim, bias=False)
            # )
            if args.share_project_in_dim:
                if latent_dim==encoder_embed_dim and model_dim==encoder_embed_dim: decoder_project_in_dim = encoder_project_in_dim  # 感觉这个才是对的，上面注释是原版条件
                else: raise ValueError("--share-project-in-dim requires --encoder-embed-dim to match both --latent-dim and --model-dim")
            else: decoder_project_in_dim=nn.Linear(latent_dim, model_dim, bias=False)
...
@register_model_architecture("difformer", "difformer_template2melody")
def difformer_template2melody(args):
    args.model_dim = getattr(args, "model_dim", 256) #
    args.latent_dim = getattr(args, "latent_dim", 110) #
    ...
    args.self_cond = getattr(args, "self_cond", True) #
    args.self_cond_before_proj = getattr(args, "self_cond_before_proj", True) #
