...
class DiffusionTransformerDecoderLayerBase(nn.Module):
    ...
    *cfg.decoder.normalize_before* to ``True``.
    ...
    def __init__(
        ...
        self.mlp = build_ffn(self.embed_dim, cfg.decoder.ffn_embed_dim, self.embed_dim, 'gelu')
    ...
    def forward(
        ...
        x = residual + gate_msa.squeeze(1) * x
        if self.attn_ln is not None:
            x = self.attn_ln(x)
        x = self.dropout_module(x)
        x = residual + gate_mlp.squeeze(1) * self.mlp(modulate(x, shift_mlp, scale_mlp))
        if not self.normalize_before:
            x = self.self_attn_layer_norm(x)
