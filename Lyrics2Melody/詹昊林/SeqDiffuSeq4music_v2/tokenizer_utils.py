...
def train_word_level_tokenizer(
    ...
    tokenizer.save(f"{str(pathlib.Path(save_path))}/word-level-vocab.json")


def train_dict_wordlevel_tokenizer(
    path: str,
    vocab_size: int = 10000,
    special_tokens=["[UNK]", "[CLS]", "[SEP]", "[PAD]", "[MASK]"],
):
    ...
    # from tokenizers.normalizers import NFD, Lowercase, StripAccents
    # from tokenizers.pre_tokenizers import Digits, Whitespace
    from tokenizers.normalizers import Replace, Strip
    from tokenizers.processors import TemplateProcessing
    from tokenizers.trainers import WordLevelTrainer
    ...
    #     [Digits(individual_digits=True), Whitespace()]
    # )
    tokenizer.normalizer = normalizers.Sequence([Strip(left=True, right=True), Replace(pattern=r'\s',content=' ')])
    tokenizer.post_processor = TemplateProcessing(
        single="[CLS] $A [SEP]", special_tokens=[("[CLS]", 1), ("[SEP]", 2)]
    )
...
def train_dict_bytelevel_tokenizer(
    path: str, #str
    vocab_size: int = 10000,
    min_frequency=1,
    special_tokens=["<s>", "<pad>", "</s>", "<unk>", "<mask>"],
    ...
    tokenizer = ByteLevelBPETokenizer()
    from tokenizers.normalizers import Replace, Strip, Sequence
    tokenizer.normalizer = Sequence([Strip(left=True, right=True), Replace(pattern=r'\s',content=' ')])
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "train-word-level":
        ...
        train_word_level_tokenizer(path=data_path, save_path=path)
    elif sys.argv[1] == "train-byte-level":
        path = f"./data/{sys.argv[2]}/"
        data_path = [path + item for item in os.listdir(path) if 'train' in item]
        train_bytelevel(path=data_path, vocab_size=int(sys.argv[3])+5, save_path=path)
    elif sys.argv[1] == "train-dict-word-level":
        train_dict_wordlevel_tokenizer(path=sys.argv[2])
    elif sys.argv[1] == "train-dict-byte-level":
        train_dict_bytelevel_tokenizer(path=sys.argv[2], vocab_size=int(sys.argv[3])+5)
